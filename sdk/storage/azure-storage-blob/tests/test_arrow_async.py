...
class TestStorageApacheArrowAsync(AsyncStorageRecordedTestCase):
    ...
    async def test_arrow_mock_xml_response(self):
        ...
        with patch.object(
            ...
            blob_pages = container_client.list_blobs(response_format="arrow", results_per_page=3).by_page()
    ...
    async def test_arrow_mock_walk_xml_response(self):
        ...
        with patch.object(
            ...
            blob_pages = container_client.walk_blobs(response_format="arrow", results_per_page=3).by_page()
            mock_deserializer = MagicMock(side_effect=deserializer_side_effect)
            blob_pages._deserializer = mock_deserializer
            first_blobs_list = [blob async for blob in await blob_pages.__anext__()]
            self.verify_blobs(first_blobs_list, blob_names[0:3])
